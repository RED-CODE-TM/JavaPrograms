...
public class CropInventoryManager {
  ...
  System.out.println("1. Add New Crop");
   ...
   private static void addNewCrop() {
    ...
    private static void updateCropQuant() {
        System.out.print("Enter the name of the crop to update: ");
        String cropToUpdate = sscrop.next();
        int index = -1;
        for (int i = 0; i < crops.length; i++) {
            if (crops[i].equalsIgnoreCase(cropToUpdate)) {
                index = i;
                break;
            }
        }
     ...
     if (index != -1) {
            System.out.print("Enter the new quantity for " + cropToUpdate + ": ");
            int newQuantity = sscrop.nextInt();
            quantities[index] = newQuantity;
            System.out.println(cropToUpdate + " quantity updated to " + newQuantity);
        } else {
